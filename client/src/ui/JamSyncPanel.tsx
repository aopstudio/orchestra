 ...
 *
 * 低频面板,默认折叠以缩短左列;折叠时标题行显示状态点(倒计时/演奏中)。
 */
...
export default function JamSyncPanel({
  ...
  const [open, setOpen] = useState(() => localStorage.getItem('orch.panel.jam') === '1')
  useEffect(() => {
    localStorage.setItem('orch.panel.jam', open ? '1' : '0')
  }, [open])
      ...
      <h2 className="panel-title">
        ...
        >
          ...
          {(countdown !== null || active) && (
            <span className="panel-title-dot" data-testid="jam-panel-status" />
          )}
        </button>
      </h2>
