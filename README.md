...
- ✅ **多乐器合奏曲目**：曲库 12 首，其中 7 首为多声部合奏——四乐器合奏（铃儿响叮当：小号+钢琴+贝斯+鼓；友谊地久天长：小号+小提琴+贝斯+鼓）、弦乐对位（卡农：双小提琴+低音）、鼓+贝斯+键盘（摇滚循环 / 十二小节布鲁斯）、铜管编制（号角合奏）、弦乐编制（弦乐小夜曲）；旋律+低音的童谣曲目保留给新手
...
- **多人合奏编排**：多人合奏内置曲目时——每人**认领一个声部（同一声部互斥）**，全部「准备就绪」后点「开始倒计时」，全房间同步起奏，各人界面显示**自己的声部**引导；单人练习同样走认领→准备→开始流程
- **引导模式**：选一首歌和一个声部，屏幕会提示现在该按哪个键（下落音符滚动条 / 琴键与鼓垫高亮 / 谱面跟随），并实时判定打分
- **自由合奏同步起奏**：不选曲也能一起开演——设置预备小节数（1–4 小节）和起始方式（小节开始强拍 / 弱起），服务器广播统一目标拍；节拍倒计时**大号悬浮在键盘正上方**（演奏者余光可见），到 0 闪 GO!（自由合奏）/「开始!」（歌曲）一起开演
- **自由合奏鼓音色**：选鼓时钢琴键让位给**全部 GM 鼓垫**（Kick/Snare/Hat/嗵鼓/Crash…带键位提示），可点击、可键盘，其他玩家的敲击在鼓垫上实时点亮
