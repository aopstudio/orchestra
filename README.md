...
- 🚧 **Phase 1 — MVP**（进行中）：房间系统、键盘/鼠标输入、双引导模式（下落音符 / 虚拟乐器高亮）、SoundFont 音色、节拍网格同步、内置曲库、判定计分
- ⏳ **Phase 2**：Web MIDI 输入、OSMD 总谱/分谱渲染与跟随、互联网部署（WSS + 房间码）、曲库扩充与曲目编辑器
- ⏳ **Phase 3**：曲库平台（UGC 上传/分享/评分）、教学引导内容、录音回放、更多编制模板（铜管/小型管弦）

详细规划见 `plan-v2.md`；Phase 0 实施细节见 `docs/plans/2026-08-11-phase0-sync-prototype.md`。

## 演奏方式

- **键盘**：白键 A–K、黑键 W–U，映射到 MIDI 音符
- **鼠标/触摸**：点击 JamPad 上的琴键
- **引导模式**：选一首歌和一个声部，屏幕会提示现在该按哪个键（下落音符滚动条 + 琴键高亮），并实时判定打分
- **进阶（Phase 2）**：Web MIDI 键盘
