...
- **空房间回收宽限期**: 最后一个玩家离开后,房间保留 **30 分钟**(默认)供重连/朋友加入;
  期间有人加入则取消回收,超时未加入则删除房间并释放房间码。
  可用 `ROOM_RECLAIM_MS` 覆盖(毫秒)。
- **曲目分享 TTL 与上限**: 分享码默认 **24 小时**过期(访问时懒清理),
  存储上限 **2000 首**(超出淘汰最旧)。可用 `SONG_SHARE_TTL_MS` / `SONG_SHARE_MAX` 覆盖。
- **死连接清理**: 服务器 30s 心跳,超时未响应(pong)的连接自动终止并触发空房回收。
- 回收均有日志(`[room] <code> reclaimed ...`),便于运维观察。
